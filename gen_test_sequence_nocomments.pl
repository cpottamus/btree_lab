...
#
# No deletes required for this quarter
...
sub gen_insert_exists {
  return "INSERT ".MakeExistentKey()." ".MakeValue();
}

sub gen_update_new {
  return "UPDATE ".MakeNonExistentKey()." ".MakeValue();
}
...
sub gen_delete_new {
  return "DELETE ".MakeNonExistentKey();
}
...
sub gen_lookup_new {
  return "LOOKUP ".MakeNonExistentKey();
}
